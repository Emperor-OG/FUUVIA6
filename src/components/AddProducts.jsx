...
const AddProductModal = ({ storeId, onProductAdded, isOpen, onClose, setLoading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const variantsStock = variants.map(v => v.stock || "0"); // ✅ remove base stock from array
      ...
      <div className="add-product-modal">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="variation-block">
            ...
            {baseVariation.preview && <img src={baseVariation.preview} alt="preview" className="preview" />}

            <input type="text" placeholder="Variation Name" value={baseVariation.name} onChange={(e) => handleBaseChange("name", e.target.value)} />

            <input type="number" placeholder="Price" value={baseVariation.price} onChange={(e) => handleBaseChange("price", e.target.value)} min="0" required />

            <input type="number" placeholder="Stock" value={baseVariation.stock} onChange={(e) => handleBaseChange("stock", e.target.value)} min="0" required />
          </div>
          ...
          <div className="variant-section">
            ...
            {variants.map((v, i) => (
              <div key={i} className="variant-block">
                ...
                <input type="text" placeholder="Name" value={v.name} onChange={(e) => handleVariantChange(i, "name", e.target.value)} required />
                <input type="number" placeholder="Price" value={v.price} onChange={(e) => handleVariantChange(i, "price", e.target.value)} min="0" required />
                <input type="number" placeholder="Stock" value={v.stock} onChange={(e) => handleVariantChange(i, "stock", e.target.value)} min="0" required />

                <button type="button" onClick={() => removeVariant(i)}>Remove</button>
              </div>
            ))}
            <button type="button" className="add-variant-btn" onClick={addVariant}>+ Add Variation</button>
          </div>
